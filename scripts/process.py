...
class EnormousBookCorpus(Preprocess):
    def __init__(self):
        ...
        dataset_folder = Path("/Volumes/T7/dataset/AIHUB/029.대규모 구매도서 기반 한국어 말뭉치 데이터/01.데이터/1.Training/라벨링데이터/TL_unscramble")
        self.result_dir = Path("/Volumes/T7/dataset/processed/corpus.txt")
        self.result_dir.parent.mkdir(exist_ok=True)
...
class Expertise(Preprocess):
    def __init__(self):
        dataset_folder = Path("../../dataset/전문분야 말뭉치/corpus/")
...
class Essay(Preprocess):
    def __init__(self):
        dataset_folder = Path("/Volumes/T7/dataset/AIHUB/024.에세이 글 평가 데이터/01.데이터/1.Training/라벨링데이터/")
        self.result_dir = Path("/Volumes/T7/dataset/processed/corpus.txt")
        self.result_dir.parent.mkdir(exist_ok=True)
